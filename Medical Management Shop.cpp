//  Medical Shop Management System by Pooja Kapri 
#include <iostream>
#include <stdlib.h>
#include <string>
#include <cctype>
...
{
public:
    void take_order(); // member function
    void delete_order();  
    void modify(); 
    void receipt(); 
    void daily_summary(); 
...
struct node //constract node *// store info related to medicine
{
	int order_no ;
	string customerName;
	string date;
	int quantity[14]; // representing the quantities of medicines ordered can store up to 14
	string type = {"OTC"}; // type of medicine ordered , intially initialized to OTC
	int x, menu2[14]; // x for number of medicines ordered //  representing the selected medicine IDs in the order
    double amount[14]; // total amount for each medicine in the order
	...
	double Medicine[14] = {50.00,30.00,20.00,10.00,20.00,70.00,80.00,90.00,40.00,100.00,70.00,30.00,110.00,120.00}; // price of medicine
	double total; // represent total bill amount
	node *next; // it points to the next node in the list when link list is created

}*q, *temp;				  // global pointers for traversing , manupulating the linked list

node *start_ptr = NULL; // like head pointer (starting point of the linked list)

 Order::Order ( )   //constructor for class Order
{
...
int main()	 
{
    system("COLOR 50"); // to change the background and text colors of the console
	Order medicine; // object of class order
	int menu;
	do
	{
		system("cls"); // clear screen ,  to clear the console screen or terminal window
...
{ 
    ...
    temp->next=NULL; // indicating this is the last node in the LL
	if(start_ptr!=NULL) // it means LL is not empty
	{
		temp->next=start_ptr;
	}
	start_ptr=temp;
